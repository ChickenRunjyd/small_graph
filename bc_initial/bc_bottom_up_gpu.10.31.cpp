...
__global__ void bc_all(index_t * dev_beg_pos, index_t * dev_csr, path_t *dev_weight, path_t * dev_bc, index_t * dev_sa_global, index_t * dev_sp_count_global, path_t * dev_dist_global)
{
    //printf("block_dim = %d\n", blockDim.x);
...
void bc_gpu_launch(graph *g)
{
    ...
    cudaMalloc( (void **) &dev_sp_count_global, V*V*sizeof(index_t));
    ...
    cudaMemset(dev_sp_count_global, 0, V*V*sizeof(index_t));
    cudaMemset(dev_dist_global, 0, V*V*sizeof(index_t));
    cudaMemset(dev_bc_global, 0, V*V*sizeof(index_t));
    
    bc_all<<<blocknum, V>>>(dev_beg_pos, dev_csr, dev_weight, dev_bc_global, dev_sa_global, dev_sp_count_global, dev_dist_global);//, dev_flag_traverse);
    cudaDeviceSynchronize();
    //bc_merge<<<1, V>>>(dev_bc);
    cudaDeviceSynchronize();
    
    cudaMemcpy(sa_global, dev_sa_global, V*V*sizeof(index_t), cudaMemcpyDeviceToHost);
    cudaMemcpy(sp_count_global, dev_sp_count_global, V*V*sizeof(index_t), cudaMemcpyDeviceToHost);
