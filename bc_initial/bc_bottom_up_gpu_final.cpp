...
const char result_file[] = "/home/yuede/small_graph/result_bc/bc_bottom_up_gpu.result";
...
__global__ void bc_merge(path_t * dev_bc_global)
{
    int thread_id = threadIdx.x;
    int tmp_id = thread_id + V;//blockDim.x;
    //printf("%d\n", );
    while(tmp_id < V*V)//blockDim.x * blockDim.x)
    {
        dev_bc_global[thread_id] += dev_bc_global[tmp_id];
        tmp_id += V;//blockDim.x;
        __syncthreads();
    }
    __syncthreads();
