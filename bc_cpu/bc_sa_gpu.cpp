...
__global__ void bc_all(index_t * dev_beg_pos, index_t * dev_csr, path_t *dev_weight, path_t * dev_bc, bool *flag)
{
    ...
    sssp<<<1, V>>>(id, dev_sssp, dev_sp_count, dev_beg_pos, dev_csr, dev_weight, dev_dist, flag);
    for(index_t i=0; i<V; ++i)
        printf("%g\n", dev_dist[i]);
    //bc_one<<<1, V>>>(id, dev_beg_pos, dev_csr, dev_weight, dev_sp_count, dev_bc, dev_bc_tmp);
...
void bc_gpu_launch(graph *g)
{
    ...
    bc_all<<<V, V>>>(dev_beg_pos, dev_csr, dev_weight, dev_bc, dev_flag_traverse);
