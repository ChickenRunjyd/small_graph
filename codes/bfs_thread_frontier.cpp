...
__global__ void traverse_one(int level, int * dev_sa, index_t * dev_beg_pos, index_t * dev_csr, bool * dev_flag)
{
...
void bfs_sa(int root, graph *g)
{
    ...
    bool * dev_flag;
    bool * flag;
    cudaHostAlloc((void **) &flag, sizeof(bool), cudaHostAllocWriteCombined);
    cudaHostGetDevicePointer( &dev_flag, flag, 0);
    * flag = true;
    while(*flag)
    {
        *flag = false;
        traverse_one<<<1, v>>>(level, dev_sa, dev_beg_pos, dev_csr, dev_flag);
        ++level;
